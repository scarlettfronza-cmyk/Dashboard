...
const META_APP_ID = required("META_APP_ID");
const META_APP_SECRET = required("META_APP_SECRET");
const OAUTH_STATE_SECRET = required("OAUTH_STATE_SECRET");
...
export async function buildMetaOAuthUrl(clientId: number, managerId: number | null = null): Promise<string> {
  ...
  const redirectUri = `${PUBLIC_BASE_URL}/api/meta/callback`;
  ...
  return `https://www.facebook.com/${META_API_VERSION}/dialog/oauth?client_id=${META_APP_ID}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=${encodeURIComponent(scope)}&state=${state}&response_type=code`;
}
...
export function registerMetaOAuthRoutes(app: Express) {
  app.get("/api/meta/callback", async (req: Request, res: Response) => {
    ...
    if (req.query.error) return res.redirect(`${PUBLIC_BASE_URL}/settings?meta_error=access_denied`);
    ...
    try { state = await verifyState(stateRaw); }
      ...
      return res.redirect(`${PUBLIC_BASE_URL}/settings?meta_error=state_expirado`);
    }

    try {
      const redirectUri = `${PUBLIC_BASE_URL}/api/meta/callback`;
      const shortToken = await axios.get(`https://graph.facebook.com/${META_API_VERSION}/oauth/access_token`, { params: { client_id: META_APP_ID, client_secret: META_APP_SECRET, redirect_uri: redirectUri, code }, timeout: 15_000 });
      const longToken = await axios.get(`https://graph.facebook.com/${META_API_VERSION}/oauth/access_token`, { params: { grant_type: "fb_exchange_token", client_id: META_APP_ID, client_secret: META_APP_SECRET, fb_exchange_token: shortToken.data.access_token }, timeout: 15_000 });
      ...
      return res.redirect(`${PUBLIC_BASE_URL}${state.returnPath}?meta_connected=pending&client=${state.clientId}`);
    } catch (error: any) {
      console.error("[Meta OAuth] callback falhou:", error?.message);
      return res.redirect(`${PUBLIC_BASE_URL}${state.returnPath}?meta_error=callback_failed`);
    }
  });
}
